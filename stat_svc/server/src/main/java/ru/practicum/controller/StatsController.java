...
public class StatsController {
    ...
    public ResponseEntity<String> postHit(@Valid @RequestBody EndpointHitDto hitDto) {
        log.info("Statistic service: Выполняется запрос Post/hit для сохранения запроса эндпоинта {}", hitDto.getUri());
    ...
    public List<EndpointStats> getStats(@RequestParam(name = "start") String start,
        ...
        log.info("Statistic service:Выполняется запрос Get/stats для статистики эндпоинтов {}", uris);
